...
[ -z "${var__is_verbose}" ] && var__is_verbose="y"
[ -z "${var__cmd_type}" ] && var__cmd_type="type__init_os"
# [ -z "${var__env_path}" ] && var__env_path="$HOME/.profile"
# [ -z "${var__inst_pkg_path}" ] && var__inst_pkg_path="$HOME/Downloads/_shares"
# [ -z "${var__inst_exec_path}" ] && var__inst_exec_path="/opt"
[ -z "${var__init_os__virtualbox_guest__apply}" ] && var__init_os__virtualbox_guest__apply="y"
[ -z "${var__init_os__virtualbox_guest__nat_host_name_local}" ] && var__init_os__virtualbox_guest__nat_host_name_local="host.mynat.local"
[ -z "${var__init_os__virtualbox_guest__nat_host_ip}" ] && var__init_os__virtualbox_guest__nat_host_ip="10.0.2.2"
[ -z "${var__init_os__proxy__apply}" ] && var__init_os__proxy__apply="y"
[ -z "${var__init_os__proxy__ip}" ] && var__init_os__proxy__ip="10.41.77.158"
[ -z "${var__init_os__proxy__port}" ] && var__init_os__proxy__port="8080"
[ -z "${var__init_os__proxy__username}" ] && var__init_os__proxy__username=""
[ -z "${var__init_os__proxy__password}" ] && var__init_os__proxy__password=""
[ -z "${var__init_os__proxy__host_name_local}" ] && var__init_os__proxy__host_name_local="internet.myproxy.local"
...
do__inst_dev() {
  while true; do
    ...
    if [ "" != "${var__inst_pkg_path}" ]; then
      mkdir -p "${var__inst_pkg_path}" || break
    fi
    if [ "" != "${var__inst_exec_path}" ]; then
      mkdir -p "${var__inst_exec_path}" || break
    fi
...
main() {
  # cd $(cd `dirname $0` && pwd)
  while true; do
    if [ "type__init_os" = "${var__cmd_type}" ]; then
      do__init_os "$@" || break
    elif [ "type__inst_dev" = "${var__cmd_type}" ]; then
      do__inst_dev "$@" || break
    else
